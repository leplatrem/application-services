...
mod tests {
    ...
    fn test_records_canonicaljson_serialization() {
        let bytes = serialize_data(1337, vec![RemoteSettingsRecord{last_modified: 42, id: "bonjour".into(), deleted: false, attachment: None, fields: json!({"foo": "bar"}).as_object().unwrap().clone()}]).unwrap();
    ...
    fn test_records_canonicaljson_serialization_with_attachment() {
        let bytes = serialize_data(1337, vec![RemoteSettingsRecord{last_modified: 42, id: "bonjour".into(), deleted: true, attachment: Some(Attachment {
            filename: "pix.jpg".into(),
            mimetype: "image/jpeg".into(),
            location: "folder/file.jpg".into(),
            hash: "aabbcc".into(),
            size: 1234567,
        }), fields: json!({}).as_object().unwrap().clone()}]).unwrap();
        let s = String::from_utf8(bytes).unwrap();
